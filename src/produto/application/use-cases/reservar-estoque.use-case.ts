  ...
  implements UseCase<ReservarEstoqueInput, Produto>
{
  ...
  async execute(input: ReservarEstoqueInput): Promise<Produto> {
    // A checagem de disponibilidade (produto.reserve) roda dentro da transacao
    // com a linha travada — ver PrismaEstoqueUnitOfWork.mutarComMovimentacao.
    return this.uow.mutarComMovimentacao({
      produtoId: input.id,
      tipo: TipoMovimentacaoEstoque.RESERVA,
      quantidade: input.quantidade,
      ctx: input.ctx ?? {},
      aplicar: (produto) => produto.reserve(input.quantidade),
    });
  }
}
