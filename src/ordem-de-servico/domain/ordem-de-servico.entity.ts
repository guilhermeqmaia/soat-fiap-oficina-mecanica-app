...
import { ItemProduto, ItemProdutoProps } from './value-objects/item-produto.vo';
...
export class OrdemDeServico {
  ...
  private _itensProduto: ItemProduto[];
    ...
    return new OrdemDeServico(
      {
        ...
        itensProduto: (props.itensProduto ?? []).map((i) => new ItemProduto(i)),
      },
      props.id,
    );
  }
  ...
  adicionarProduto(props: ItemProdutoProps): ItemProduto {
    this.ensureStatusEmDiagnostico();

    if (this._itensProduto.some((i) => i.produtoId === props.produtoId)) {
      throw new ProdutoDuplicadoNaOSError(props.nomeProduto);
    }

    const item = new ItemProduto(props);
    this._itensProduto.push(item);
    return item;
  }

  removerProduto(produtoId: string): ItemProduto {
    this.ensureStatusEmDiagnostico();

    const index = this._itensProduto.findIndex((i) => i.produtoId === produtoId);
    if (index === -1) {
      throw new ProdutoInexistenteNaOSError(produtoId);
    }

    const [removido] = this._itensProduto.splice(index, 1);
    return removido;
  }

  private ensureStatusEmDiagnostico(): void {
    if (!this._status.equals(StatusOSVO.create(StatusOS.EM_DIAGNOSTICO))) {
      throw new InvalidStatusTransitionError(
        this._status.toString(),
        StatusOS.EM_DIAGNOSTICO,
  ...
  get itensProduto(): ReadonlyArray<ItemProduto> {
    return this._itensProduto;
  }

  get valorTotalProdutos(): number {
    return this._itensProduto.reduce((acc, item) => acc + item.valorTotal, 0);
  }
