...
const USERS: Record<string, TestUser> = {
  admin:     { nome: 'Admin',     email: 'admin.os@oficina.com',     senha: 'admin123',     role: Role.ADMIN },
  atendente: { nome: 'Atendente', email: 'atendente.os@oficina.com', senha: 'atend123',     role: Role.ATENDENTE },
  mecanico:  { nome: 'Mecanico',  email: 'mecanico.os@oficina.com',  senha: 'mec123',       role: Role.MECANICO },
  estoquista:{ nome: 'Estoquista',email: 'estoquista.os@oficina.com',senha: 'estq123',      role: Role.ESTOQUISTA },
  cliente:   { nome: 'Cliente',   email: 'cliente.os@oficina.com',   senha: 'cli123',       role: Role.CLIENTE },
};
...
describe('OrdemDeServico (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const user of Object.values(USERS)) {
      ...
      const created = await prisma.usuario.create({
        data: { nome: user.nome, email: user.email, senhaHash, role: user.role, ativo: true },
      });
    ...
    const clienteRes = await request(app.getHttpServer())
      ...
      .send({ nome: 'Joao Silva', cpfCnpj: '529.982.247-25', telefone: '11999990000', email: USERS.cliente.email });
    ...
    const veiculoRes = await request(app.getHttpServer())
      .post('/veiculos')
      .set('Authorization', `Bearer ${tokens.admin}`)
      .send({ placa: 'TES1T23', marca: 'Toyota', modelo: 'Corolla', ano: 2024, clienteId });
    ...
    const servicoRes = await request(app.getHttpServer())
      .post('/servicos')
      .set('Authorization', `Bearer ${tokens.admin}`)
      .send({ nome: 'Troca de pastilha', precoBase: 200, tempoEstimadoHoras: 1 });
  ...
  describe('POST /ordens-servico', () => {
    ...
    it('should return 409 when veiculo does not belong to cliente', async () => {
      const outroCliente = await request(app.getHttpServer())
        .post('/clientes')
        .set('Authorization', `Bearer ${tokens.admin}`)
        .send({ nome: 'Outro Cliente', cpfCnpj: '836.076.971-08', telefone: '11000000001' });
  ...
  describe('GET /ordens-servico', () => {
    ...
    it('should filter by status', async () => {
      ...
      expect(res.status).toBe(200);
      res.body.data.forEach((os: any) => expect(os.status).toBe(StatusOS.RECEBIDA));
    });
  ...
  describe('Full lifecycle — approval path (RECEBIDA → ENTREGUE)', () => {
    ...
    it('4. completes diagnosis → AGUARDANDO_APROVACAO', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${tokens.mecanico}`)
        .send({ diagnostico: 'Pastilhas de freio totalmente desgastadas, troca urgente' });
  ...
  describe('Full lifecycle — rejection path (AGUARDANDO_APROVACAO → CANCELADA)', () => {
    ...
    it('1. creates OS and progresses to AGUARDANDO_APROVACAO', async () => {
      ...
      const diagRes = await request(app.getHttpServer())
        .post(`/ordens-servico/${osId}/completar-diagnostico`)
        .set('Authorization', `Bearer ${tokens.admin}`)
        .send({ diagnostico: 'Motor com desgaste severo, reparo muito custoso' });
  ...
  describe('Service management on OS', () => {
    ...
    it('should return 404 when adding a non-existent service', async () => {
      const res = await request(app.getHttpServer())
        .post(`/ordens-servico/${osId}/servicos`)
        .set('Authorization', `Bearer ${tokens.admin}`)
        .send({ servicoId: '00000000-0000-0000-0000-000000000000', quantidade: 1 });
    ...
    it('should remove an added service', async () => {
      ...
      const osRes = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${tokens.admin}`);
      expect(osRes.body.itensServico).toHaveLength(0);
    });
