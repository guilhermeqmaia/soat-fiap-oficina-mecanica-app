...
/**
 * Use Case: RemoverProdutoDaOS
 * Camada: Application — BC Atendimento
 *
 * Fluxo:
 *  1. Valida existência da OS
 *  2. Remove item (domínio valida status e existência do item, emite ProdutoRemovidoDaOS)
 *  3. Estorna reserva no estoque
 *  4. Persiste OS
 */
export class RemoverProdutoDaOS {
  ...
  async executar({ ordemDeServicoId, produtoId }: RemoverProdutoDaOSCommand): Promise<RemoverProdutoDaOSResult> {
    ...
    // (Opcional) publicar domain events
